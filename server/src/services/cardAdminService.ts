...
const targets = Object.values(CardModifierTarget)
...
function assertModifier(input: Record<string, unknown>) {
  ...
  if (!targets.includes(input.target as CardModifierTarget) || !Object.values(CardModifierDirection).includes(input.direction as CardModifierDirection) || !Object.values(CardModifierOperation).includes(input.operation as CardModifierOperation) || !Number.isInteger(input.value) || Number(input.value) < 0 || Number(input.value) > 100) throw Object.assign(new Error('Modificateur invalide.'), { statusCode: 400 })
}
...
export async function createModifier(slug: string, input: Record<string, unknown>) { assertCard(slug); assertModifier(input); return prisma.cardModifier.create({ data: { cardSlug: slug, name: input.name as string, description: input.description as string, target: input.target as CardModifierTarget, direction: input.direction as CardModifierDirection, operation: input.operation as CardModifierOperation, value: Number(input.value), condition: typeof input.condition === 'string' ? input.condition : null, active: input.active !== false } }) }
