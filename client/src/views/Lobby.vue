...
async function refreshLobby() { if (!auth.token || pollInFlight || lobby.value?.status === 'PLAYING') return; pollInFlight = true; try { lobby.value = await getGameLobby(auth.token, String(route.params.id)); error.value = '' } finally { pollInFlight = false } }
...
async function start() { if (!auth.token || !lobby.value || !lobby.value.canStart || lobby.value.creatorId !== auth.user?.id) return; starting.value = true; error.value = ''; try { const started = await startGameLobby(auth.token, lobby.value.id); lobby.value = started.lobby; if (pollTimer) clearTimeout(pollTimer); await router.push({ path: `/partie/${started.lobby.id}`, query: { mode: started.lobby.mode } }) } catch (exception) { error.value = exception instanceof Error ? exception.message : 'Impossible de démarrer le combat.' } finally { starting.value = false } }
</script>

<template>
  <main class="lobby-page"><SocialHeader /><section class="lobby-content"><p class="eyebrow">Salon de combat</p><p v-if="error" class="social-error">{{ error }}</p><template v-else-if="lobby"><h1>{{ lobby.mode }}</h1><p class="lobby-state">{{ lobby.status === 'READY' ? 'Tous les joueurs ont accepté' : lobby.status === 'PLAYING' ? 'Combat en cours' : 'En attente' }} · {{ lobby.playerCount }} / {{ lobby.expectedPlayers }} joueurs</p><div class="lobby-players"><article v-for="player in lobby.players" :key="player.isAi ? 'ai' : player.id"><strong>{{ player.displayName }}<span v-if="player.isAi" class="ai-badge">IA</span></strong><span>{{ player.status === 'ACCEPTED' ? 'Accepté' : player.status === 'REJECTED' ? 'Refusé' : 'En attente' }}</span></article></div><button v-if="lobby.canStart && lobby.creatorId === auth.user?.id" class="start-button" type="button" :disabled="starting" @click="start">{{ starting ? 'DÉMARRAGE...' : 'ENTAMER LE COMBAT' }}</button><template v-else-if="lobby.canStart"><p class="lobby-empty">Le créateur peut maintenant lancer le combat.</p></template><button v-if="lobby.status === 'PLAYING'" class="start-button" type="button" @click="router.push({ path: `/partie/${lobby.id}`, query: { mode: lobby.mode } })">REJOINDRE LE COMBAT</button><p v-if="lobby.status === 'WAITING'" class="lobby-empty">La partie commencera lorsque tous les invités auront accepté.</p></template><p v-else class="lobby-empty">Chargement du salon...</p></section></main>
</template>
