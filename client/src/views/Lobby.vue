...
async function start() { if (!auth.token || !lobby.value || lobby.value.status !== 'READY' || lobby.value.creatorId !== auth.user?.id) return; starting.value = true; error.value = ''; try { const started = await startGameLobby(auth.token, lobby.value.id); lobby.value = started.lobby; if (pollTimer) clearTimeout(pollTimer); await router.push({ path: `/partie/${started.lobby.id}`, query: { mode: started.lobby.mode } }) } catch (exception) { error.value = exception instanceof Error ? exception.message : 'Impossible de démarrer le combat.' } finally { starting.value = false } }
</script>
