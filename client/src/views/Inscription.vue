...
<template>
  <main class="auth-page"><nav class="auth-nav"><a class="brand" href="/"><img class="brand-logo" src="/logo.png" alt="Shinobi Area" /></a><a class="profile-link" href="/connexion">Se connecter</a></nav><form class="auth-panel" @submit.prevent="submit"><p class="eyebrow">Nouveau shinobi</p><h1>Inscription</h1><label>Nom / pseudo<input v-model.trim="displayName" type="text" autocomplete="nickname" required /></label><label>Email<input v-model.trim="email" type="email" autocomplete="email" required /></label><label>Mot de passe<input v-model="password" type="password" autocomplete="new-password" minlength="6" required /></label><p v-if="error" class="auth-error">{{ error }}</p><button class="auth-submit" type="submit" :disabled="loading">{{ loading ? 'Création...' : 'Créer le compte' }}</button></form></main>
</template>
